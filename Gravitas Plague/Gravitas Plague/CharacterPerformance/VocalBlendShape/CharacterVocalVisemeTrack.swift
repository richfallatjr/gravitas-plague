...
nonisolated struct CharacterVocalVisemeTrackIdentity:
    Sendable,
    Hashable,
    Equatable
{
    ...
    func matches(_ playback: CharacterVocalPlaybackIdentity) -> Bool {
        characterID == playback.characterID &&
            role == playback.role &&
            audioFile == playback.fileName &&
            looping == playback.isLooping
    }
}

nonisolated struct CharacterVocalVisemeTrack: Sendable, Equatable {
    ...
    func pose(atFrame frame: Int, cursor: inout Int) -> MindEyeMouthPose {
        ...
        if runs.indices.contains(cursor) {
            ...
            if frame >= current.endFrameExclusive {
                ...
                if frame >= advanced.startFrame,
                   frame < advanced.endFrameExclusive {
