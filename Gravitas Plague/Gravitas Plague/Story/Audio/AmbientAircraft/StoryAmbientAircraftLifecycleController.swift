...
nonisolated enum StoryAmbientAircraftSuppression: String, Hashable, Sendable {
    case titleCard
    case operationModeTeardown
    case immersiveShutdown
}

nonisolated struct StoryAmbientAircraftLifecycleState: Sendable, Equatable {
    private(set) var storyPropsCommitted = false
    private(set) var terminalAngelSequenceBegan = false
    private(set) var suppressions = Set<StoryAmbientAircraftSuppression>()
    ...
    mutating func deactivate(_ suppression: StoryAmbientAircraftSuppression) {
...
@MainActor
final class StoryAmbientAircraftLifecycleController {
    private let scheduler: StoryAmbientAircraftScheduler
    private let worldBridge: StoryAmbientAircraftWorldBridge
    private var state = StoryAmbientAircraftLifecycleState()
    private var reconcileRevision = 0
    private var reconcileTask: Task<Void, Never>?

    init(
        scheduler: StoryAmbientAircraftScheduler,
    ...
    func finalAngelDeathSequenceBegan(reason: String) {
        ...
        print(
            "[StoryAmbientAircraft] terminal cutoff latched " +
