...
@MainActor
final class TuringStoryHamReceiverIconController {
    ...
    func install(
        ...
        remove()
        TuringStoryHamReceiverActionComponents
            .registerIfNeeded()
        ...
        icon.name =
            "TuringStoryHamReceiver_ActionIcon"
        icon.position = .zero
        ...
        print("""
        [TuringHamReceiver] action targets installed
    ...
    private func makePhysicalHitTarget(
        ...
        let bounds =
            hamReceiverRoot.visualBounds(
                recursive: true,
                relativeTo: hamReceiverRoot,
        ...
        let target = Entity()
        target.name =
            "TuringStoryHamReceiver_PhysicalHitTarget"
        ...
        hamReceiverRoot.addChild(target)
        return target
    }
    ...
    private func updateIcon(
        ...
        model.materials = [
            material(symbolName: symbolName)
        ]
    ...
    private func symbolTexture(
        ...
        guard let symbol = UIImage(
            ...
            throw TuringRuntimeError.invalidConfig(
                "Unable to render ham-receiver icon \(symbolName)."
            )
        }
        let size = CGSize(
            width: 256,
        ...
        guard let cgImage = image.cgImage else {
            throw TuringRuntimeError.invalidConfig(
                "Unable to rasterize ham-receiver icon \(symbolName)."
            )
        }
        return try TextureResource(
            image: cgImage,
            withName:
                "turing_ham_receiver_\(symbolName)",
