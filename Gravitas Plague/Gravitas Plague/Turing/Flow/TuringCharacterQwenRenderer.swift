    ...
    -> TuringCharacterRenderReport
  {
    ...
    do {
      ...
      let bundledModel =
        ...
        .locateBundledModel()
      let stagedModel =
        try resources.stageWritableModel(
          from: bundledModel
        )
      ...
      try await freshPool
        .warmLoadExactlyRequestedInstances(
      ...
      let windowStrategy: TuringQwenNativeReferenceWindowStrategy
      ...
      let samplingPolicy =
        runtime.qwen.samplingPolicy
      let qualityPolicy =
        runtime.qwen
        .generationQualityPolicy

      let requests =
        segments.enumerated().map {
          index,
          segment in

          let samplingSeed =
            TuringQwenNativeSamplingSeed
            .make(
              voiceID:
                runtime.voiceID,
              runID: runID,
      ...
      print(
        ...
        [TuringFlow] Fresh2 pool ready
          ...
          talkerSamplingMode: \(samplingPolicy.talker.mode.rawValue)
          talkerSamplingBackend: \(samplingPolicy.talker.backend.rawValue)
          talkerTemperature: \(samplingPolicy.talker.temperature)
          talkerTopK: \(samplingPolicy.talker.topK)
          talkerTopP: \(samplingPolicy.talker.topP)
          talkerRepetitionPenalty: \(samplingPolicy.talker.repetitionPenalty)
          codePredictorSamplingMode: \(samplingPolicy.codePredictor.mode.rawValue)
          codePredictorSamplingBackend: \(samplingPolicy.codePredictor.backend.rawValue)
          requireEOSBeforeDecode: \(qualityPolicy.requireEOSBeforeDecode)
          minimumPeakAbs: \(qualityPolicy.minimumPeakAbs)
          minimumRMS: \(qualityPolicy.minimumRMS)
        """)
