...
struct TuringCharacterRuntimeDefinition: Codable, Sendable, Hashable {
    ...
    struct OutputProcessing: Codable, Sendable, Hashable {
        let playbackRate: Double
    }

    struct Qwen: Codable, Sendable, Hashable {
        let maxNewRows: Int
    ...
    struct Audio: Codable, Sendable, Hashable {
    ...
    var outputProcessingPolicy: TuringQwenOutputProcessingPolicy {
        TuringQwenOutputProcessingPolicy(
            voiceID: voiceID,
            playbackRate: outputProcessing.playbackRate
        )
    }

    func supports(_ route: TuringVoiceOutputContext) -> Bool {
...
protocol TuringCharacterRuntimeProviding: Sendable {
    func require(_ characterID: String) throws
    ...
    Sendable
{
    ...
    init(
        ...
        let resource = try TuringResourceLoader.decodeResource(
        ...
        guard resource.schemaVersion == 1 else {
            throw TuringRuntimeError.invalidConfig(
                "character-runtimes schemaVersion must be 1."
            )
        }
        ...
        for definition in resource.characters {
            ...
            guard result[definition.characterID] == nil else {
                throw TuringRuntimeError.invalidConfig(
                    "Duplicate Turing character runtime: \(definition.characterID)."
                )
            }
            result[definition.characterID] = definition
        }
    ...
    init(
        definitions: [TuringCharacterRuntimeDefinition]
    ) throws {
        var result:
            [String: TuringCharacterRuntimeDefinition] = [:]
    ...
    func require(
        ...
        guard let definition = definitions[characterID] else {
            throw TuringRuntimeError.invalidConfig(
    ...
    private static func validate(
        _ definition: TuringCharacterRuntimeDefinition
    ) throws {
        let required = [
        ...
        guard definition.allowedOutputRoutes.isEmpty == false else {
            throw TuringRuntimeError.invalidConfig(
                "\(definition.characterID) must allow at least one output route."
            )
        }

        guard definition.outputProcessing.playbackRate > 0.25,
              definition.outputProcessing.playbackRate <= 2 else {
            throw TuringRuntimeError.invalidConfig(
        ...
        let strategy = definition.qwen.referenceWindowStrategy
