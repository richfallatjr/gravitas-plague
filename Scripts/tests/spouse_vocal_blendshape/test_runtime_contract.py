...
class SpouseVocalBlendShapeRuntimeContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.root = Path(__file__).resolve().parents[3]
        cls.paths = ToolPaths.discover(cls.root, profile=SPOUSE_PROFILE)
    ...
    def test_spouse_source_locks_legacy_donor_name_to_spouse_output(self):
        source = load_source_descriptor(
            self.paths.source_descriptor,
            SPOUSE_PROFILE,
        )
        self.assertEqual(source["donorBlendShapeName"], "dadVocalClose")
        self.assertEqual(source["blendShapeName"], "spouseVocalClose")
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(ValueError, "donor blendshape name"):
                load_source_descriptor(path, SPOUSE_PROFILE)
    ...
    def test_required_donor_target_never_falls_back_to_mesh_points(self):
        ...
        with patch.dict(sys.modules, {"pxr": fake_pxr}):
            with self.assertRaisesRegex(
                ValueError,
                "required donor blendshape dadVocalClose is missing",
            ):
                _resolve_donor_target(
                    stage,
    ...
    def test_generated_descriptor_payload_and_asset_contract(self):
        ...
        self.assertIn("textures/spouse_biped_ior.png", names)
    ...
    def test_report_proves_explicit_donor_resolution_and_output_target(self):
        ...
        self.assertEqual(report["characterID"], "spouse")
        self.assertFalse(report["donorBundled"])
        self.assertEqual(
            report["baseAssetSHA256Before"],
            "0577a7bb9234388e55b371df53731e68170a748e65098df68814bc4536d0eaa9",
        )
        self.assertEqual(
            report["donorAssetSHA256"],
            "18de17f3c531ed66c9ea41e3293788abde5b6b01b8b447ce7e07f25e9086d192",
        )
        mesh = report["validation"]["meshes"][0]
        self.assertEqual(
            mesh["baseTopologySHA256"],
            "3406a5fdf35535d6099a9364dc5d6169ec65a4c2c620f3f8e28e05ef231127d5",
        )
        ...
        self.assertEqual(mesh["donorBlendShapeOffsetRecordCount"], 90_368)
        self.assertEqual(mesh["donorBlendShapeNonzeroOffsetRecordCount"], 937)
        self.assertEqual(mesh["changedPointCount"], 1_550)
        self.assertAlmostEqual(
            mesh["worldDisplacementMeters"]["maximum"],
            0.02838659896631332,
        )
        authored = report["authoredAsset"]["authored"][0]
        self.assertEqual(authored["blendShapeName"], "spouseVocalClose")
        self.assertEqual(
            authored["blendShapePrimPath"],
            "/root/Armature/char1/char1_spouseVocalClose",
        )
