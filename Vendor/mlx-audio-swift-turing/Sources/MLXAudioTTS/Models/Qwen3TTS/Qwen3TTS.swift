...
public final class Qwen3TTSModel: Module, SpeechGenerationModel, @unchecked Sendable {
    ...
    func sampleToken(
        ...
        // Turing Phase 0 visionOS canary path.
        //
        // The upstream sampler performs several Metal-backed scatter/masking operations
        // (`putAlong`, `categorical`) after the model forward pass. On Apple Vision Pro
        // with Metal API Validation enabled, the current Qwen3-TTS tuple can abort the
        // process inside MTLDebugComputeCommandEncoder before Swift can throw. This
        // host-side sampler keeps the Qwen forward/code-predictor/decoder work in MLX,
        // but moves the tiny token-selection control step to Swift arrays so the app can
        // prove Phase 0 Qwen audio without encoding the known-bad scatter path.
        //
        // This is enabled by default on visionOS in the local Turing package override.
        // It can also be enabled on other platforms with Swift Active Compilation Conditions:
        //   TURING_QWEN_PHASE0_HOST_SAMPLER
        // Optional deterministic canary:
        //   TURING_QWEN_PHASE0_HOST_SAMPLER_GREEDY
        if turingPhase0UseHostSafeSampler {
            return sampleTokenHostSafe(
                logitsSlice,
                temperature: temperature,
                topP: topP,
                topK: topK,
    ...
    private var turingPhase0UseHostSafeSampler: Bool {
        #if os(visionOS)
        return true
        #elseif TURING_QWEN_PHASE0_HOST_SAMPLER
        return true
        #else
        return ProcessInfo.processInfo.environment["TURING_QWEN_PHASE0_HOST_SAMPLER"] == "1"
        #endif
    }

    private var turingPhase0ForceGreedyHostSampler: Bool {
        #if TURING_QWEN_PHASE0_HOST_SAMPLER_GREEDY
        return true
        #else
        return ProcessInfo.processInfo.environment["TURING_QWEN_PHASE0_HOST_SAMPLER_GREEDY"] == "1"
        #endif
    }

    private func sampleTokenHostSafe(
        ...
        let vocabSize = max(0, logitsSlice.dim(-1))
        let safeFallback = Int32(max(0, min(eosTokenId ?? 0, max(vocabSize - 1, 0))))

        guard vocabSize > 0 else {
            print("[Qwen3TTS][TuringHostSampler] empty vocab; returning fallback token \(safeFallback)")
            return MLXArray([safeFallback]).reshaped(1, 1)
        }

        // Force materialization before crossing into Swift memory. This is intentionally
        // after the model forward pass and before any MLX scatter/mask sampling op.
        eval(logitsSlice)

        var values = logitsSlice.asArray(Float.self)
        if values.count >= vocabSize {
            values = Array(values[0 ..< vocabSize])
        }

        guard values.count == vocabSize else {
            print("""
            [Qwen3TTS][TuringHostSampler] logits size mismatch
              expectedVocab: \(vocabSize)
              actualValues: \(values.count)
              returningFallback: \(safeFallback)
            """)
            return MLXArray([safeFallback]).reshaped(1, 1)
        }

        let veryNegative = -Float.greatestFiniteMagnitude / 16.0

        if let suppressTokens, suppressTokens.isEmpty == false {
            let eos = eosTokenId ?? -1
            for token in suppressTokens where token >= 0 && token < vocabSize && token != eos {
                values[token] = veryNegative
            }
        }

        if let generatedTokens,
           generatedTokens.isEmpty == false,
           repetitionPenalty != 1.0,
           repetitionPenalty.isFinite,
           repetitionPenalty > 0 {
            for token in Set(generatedTokens) where token >= 0 && token < vocabSize {
                if values[token] < 0 {
                    values[token] *= repetitionPenalty
                } else {
                    values[token] /= repetitionPenalty
                }
            }
        ...
        if minP > 0, minP.isFinite {
            let maxLogit = values.max() ?? veryNegative
            let threshold = maxLogit + Float(log(Double(max(minP, 1.0e-9))))
            for index in values.indices where values[index] < threshold {
                values[index] = veryNegative
            }
        }

        if topK > 0, topK < vocabSize {
            let sorted = values.sorted(by: >)
            let cutoff = sorted[max(0, topK - 1)]
            for index in values.indices where values[index] < cutoff {
                values[index] = veryNegative
            }
        }

        if topP > 0, topP < 1.0 {
            values = applyHostTopP(values, topP: topP, floor: veryNegative)
        }

        if temperature <= 0 || turingPhase0ForceGreedyHostSampler {
            let token = hostArgmax(values, fallback: safeFallback)
            return MLXArray([token]).reshaped(1, 1)
        }

        let token = hostSample(values, temperature: temperature, fallback: safeFallback)
        return MLXArray([token]).reshaped(1, 1)
    }

    private func applyHostTopP(
        _ logits: [Float],
        topP: Float,
        floor: Float
    ) -> [Float] {
        guard logits.isEmpty == false else { return logits }
        let maxLogit = logits.max() ?? 0
        let probabilities = logits.map { value -> Float in
            guard value.isFinite else { return 0 }
            return Float(exp(Double(value - maxLogit)))
        }
        let total = probabilities.reduce(Float(0), +)
        guard total.isFinite, total > 0 else { return logits }

        let ranked = probabilities.enumerated().sorted { lhs, rhs in
        ...
        var filtered = logits
        for index in filtered.indices where keep.contains(index) == false {
            filtered[index] = floor
        }
        return filtered
    }

    private func hostArgmax(
        _ logits: [Float],
        fallback: Int32
    ) -> Int32 {
        var bestIndex = Int(fallback)
    ...
    private func hostSample(
        _ logits: [Float],
        temperature: Float,
        fallback: Int32
    ) -> Int32 {
        let temp = max(temperature, 1.0e-5)
        let maxLogit = logits.filter(\.isFinite).max() ?? 0
        var weighted = [(index: Int, weight: Float)]()
        weighted.reserveCapacity(logits.count)

        for (index, logit) in logits.enumerated() where logit.isFinite {
            let scaled = (logit - maxLogit) / temp
            let weight = Float(exp(Double(scaled)))
            if weight.isFinite, weight > 0 {
                weighted.append((index, weight))
            }
        }

        let total = weighted.reduce(Float(0)) { $0 + $1.weight }
        guard total.isFinite, total > 0 else {
            return hostArgmax(logits, fallback: fallback)
        }

        var generator = SystemRandomNumberGenerator()
        let threshold = Float.random(in: 0 ..< total, using: &generator)
        var cumulative = Float(0)
        for candidate in weighted {
            cumulative += candidate.weight
            if threshold <= cumulative {
                return Int32(candidate.index)
            }
        }
        return Int32(weighted.last?.index ?? Int(fallback))
    }
